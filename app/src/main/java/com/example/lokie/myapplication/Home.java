    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (mnuslctd) {
            case "Duplicate":
                ...
                if (s1=="") {
                    duplicate.putExtra("a", s1);
                }
                else {
                    name.setError("Enter name");
                }
                if (s2=="") {
                duplicate.putExtra("b", s2);
                }
                else {
                    age.setError("Enter Age");
                }
                if (s3=="") {
                duplicate.putExtra("c", s3);
                }
                else {
                    doj.setError("Select date");
                }
                if (s4=="") {
                duplicate.putExtra("d", s4);
                }
                else {
                    timein.setError("Select Time");
                }

                duplicate.putExtra("e", s5);
                duplicate.putExtra("f", s6);
                startActivity(duplicate);
                break;
            case "Listview":
                Intent lv = new Intent(Home.this, LstVw.class);
                if (s1=="") {
                    lv.putExtra("listname", s1);
                    startActivity(lv);
                }
                else {
                    name.setError("Enter name");
